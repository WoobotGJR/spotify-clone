import {
  Close,
  Content,
  Description,
  Overlay,
  Portal,
  Root,
  Title,
} from '@radix-ui/react-dialog';
import { ReactNode } from 'react';
import { IoMdClose } from 'react-icons/io';

interface ModalProps {
  isOpen: boolean;
  onChange: (open: boolean) => void;
  title: string;
  description: string;
  children: ReactNode;
}

const Modal: React.FC<ModalProps> = ({
  isOpen,
  onChange,
  title,
  description,
  children,
}) => {
  return (
    <Root defaultOpen={isOpen} open={isOpen} onOpenChange={onChange}>
      <Portal>
        <Overlay className="bg-neutral-900/90 backdrop-blur-sm fixed inset-0" />
        <Content className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
          <Title className="text-xl text-center font-bold mb-4">{title}</Title>
          <Description className="mb-5 text-sm leading-normal text-center">
            {description}
          </Description>
          <div>{children}</div>
          <Close>
            <button className="text-neutral-400 hover:text-white absolute top-[10px] right-[10px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:outline-none">
              <IoMdClose />
            </button>
          </Close>
        </Content>
      </Portal>
    </Root>
  );
};
export default Modal;
